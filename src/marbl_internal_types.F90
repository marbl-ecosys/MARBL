...
contains
  ...
  subroutine tracer_index_constructor(this, ciso_on, autotrophs, zooplankton, &
    ...
    character(len=char_len),        intent(in)    :: auto_names(:)
    character(len=char_len),        intent(in)    :: zoo_names(:)
    ...
    associate(tracer_cnt      => marbl_total_tracer_cnt)
      ...
      ! Set up autotrophs and zooplankton before determining tracer indices
      call marbl_config_zooplankton(zooplankton, zoo_names, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_status_log%log_error_trace('marbl_config_zooplankton', subname)
        return
      end if
      call marbl_config_autotrophs(autotrophs, auto_names, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_status_log%log_error_trace('marbl_config_autotroph', subname)
        return
      end if
  ...
  subroutine marbl_config_zooplankton(zooplankton, zoo_names, marbl_status_log)

  use marbl_constants_mod, only : dps

  type(zooplankton_type),  dimension(:), intent(out)   :: zooplankton
  character(len=char_len), dimension(:), intent(in)    :: zoo_names
  type(marbl_log_type),                  intent(inout) :: marbl_status_log
  ...
  subroutine marbl_config_autotrophs(autotrophs, auto_names, marbl_status_log)

  use marbl_constants_mod, only : dps

  type(autotroph_type),    dimension(:), intent(out)   :: autotrophs
  character(len=char_len), dimension(:), intent(in)    :: auto_names
  type(marbl_log_type),                  intent(inout) :: marbl_status_log
