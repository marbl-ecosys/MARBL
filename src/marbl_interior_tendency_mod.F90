...
contains
  ...
  subroutine marbl_interior_tendency_compute( &
    ...
    call compute_function_scaling(temperature, Tfunc, Tfunc_cocco)
    ...
    call compute_autotroph_photosynthesis(km, num_PAR_subcols, autotroph_local, temperature(:), &
         Tfunc(:), Tfunc_cocco(:), PAR%col_frac(:), PAR%avg(:,:), autotroph_derived_terms)
    ...
    call compute_autotroph_loss(km, Tfunc, Tfunc_cocco, autotroph_derived_terms)
  ...
  subroutine compute_function_scaling(temperature, Tfunc, Tfunc_cocco)
    ...
    Tfunc_cocco = 0.12_r8 * (max(c0, min(temperature, 27.0_r8))**0.4_r8)
  ...
  subroutine compute_autotroph_photosynthesis(km, PAR_nsubcols, &
       autotroph_local, temperature, Tfunc, Tfunc_cocco, &
       PAR_col_frac, PAR_avg, autotroph_derived_terms)
    ...
    real(r8),                           intent(in)    :: Tfunc_cocco(km)
      ...
      do auto_ind = 1, autotroph_cnt
        do k=1,km
          ...
          if (temperature(k) < autotroph_settings(auto_ind)%temp_thres) then
            ...
            if (autotroph_settings(auto_ind)%exp_calcifier) then
              PCmax = autotroph_settings(auto_ind)%PCref * f_nut(auto_ind,k) * Tfunc_cocco(k)
            else
  ...
  subroutine compute_autotroph_loss(km, Tfunc, Tfunc_cocco, autotroph_derived_terms)
    ...
    associate(                                                         &
      ...
      do auto_ind = 1, autotroph_cnt
        ...
        if (autotroph_settings(auto_ind)%exp_calcifier) then
          auto_loss(auto_ind,:) = autotroph_settings(auto_ind)%mort * Pprime(auto_ind,:) * Tfunc_cocco(:)
        else
          ...
          do prey_ind = 1, max_grazer_prey_cnt
            ...
            select case (grazing_relationship_settings(prey_ind, pred_ind)%grazing_function)
              ...
              case (grz_fnc_michaelis_menten)
                ...
                if (work1 > c0) then
                  ! Tested with Tfunc_cocco for explicit calcifiers, but Tfunc for all gave better results
